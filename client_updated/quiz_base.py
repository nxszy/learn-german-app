    ...
    def check_content(self):
        ...
        if self.ids['check_text'].text == "Check":
            ...
            for i, id in enumerate(self.tf_ids):
                ...
                user_input = self.ids[id].text.strip('\t').strip('\n').strip()
            ...
            max_points = len(self.manager.learningSet) * self.points_per_screen
            ...
            self.manager.get_screen('resultsScreen').ids['result'].text = f"You got \n {round(self.points,2)} / {max_points} \n That's {round(self.points/max_points, 2)*100}%"
